...
export function getAnalyticsData(
  ...
  return {
    kpis: {
      revenue: {
        value: `$${revAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
        change: `+${(12.4 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: intervalLabel,
      },
      activeProjects: {
        value: activeProjs.toString(),
        change: `+${(2 * factor).toFixed(0)} new`,
        isPositive: true,
        comparisonLabel: 'started this interval',
      },
      completedProjects: {
        value: completedProjs.toString(),
        change: `+${(8.2 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: intervalLabel,
      },
      totalLeads: {
        value: totalLeadsCount.toString(),
        change: `+${(15.1 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: intervalLabel,
      },
      convertedLeads: {
        value: convertedLeadsCount.toString(),
        change: `${Math.round(38 * factor)}%`,
        isPositive: true,
        comparisonLabel: 'avg conversion rate',
      },
      pendingInvoices: {
        value: invoiceCount.toString(),
        change: `$${(8400 * factor).toLocaleString(undefined, { maximumFractionDigits: 0 })}`,
        isPositive: false,
        comparisonLabel: 'total outstanding balance',
      },
      teamProductivity: {
        value: `${prodScore}%`,
        change: `+${(1.5 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: 'efficiency index',
      },
      monthlyGrowth: {
        value: `+${(8.7 * factor).toFixed(1)}%`,
        change: `+${(1.2 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: 'growth index delta',
      },
      clientRetention: {
        value: `${rententionRate}%`,
        change: `+${(0.4 * factor).toFixed(1)}%`,
        isPositive: true,
        comparisonLabel: 'avg account continuity',
      },
      profitability: {
        value: `${profitMargin}%`,
        change: `+${(2.1 * factor).toFixed(1)}%`,
        isPositive: true,
