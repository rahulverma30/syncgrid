  ...
  return (
    <div className="flex flex-col gap-6 p-6 md:p-8 max-w-7xl mx-auto min-h-screen text-slate-100 select-none">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border/20 bg-slate-950/20 p-8 backdrop-blur-md shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-2 z-10 max-w-xl">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-mono">Module 14</span>
            <span className="text-xs text-slate-500">Enterprise Wiki & SOP Hub</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-100">Knowledge Management Center</h1>
          <p className="text-sm text-slate-400 leading-relaxed">
            Centralize standard operating procedures (SOPs), company access policies, employee onboarding guides, and collaborative project wikis in a premium Notion/Confluence styled workspace.
          </p>
        </div>
        ...
        {spaces.length === 0 && (
          <div className="z-10 flex flex-col gap-2 items-start justify-center border border-dashed border-border/40 p-4 rounded-xl bg-slate-950/40 backdrop-blur-md max-w-xs">
            <span className="text-xs text-slate-400 font-medium">Ready to explore? Populate the sandbox!</span>
            <button
              onClick={handleSeed}
              disabled={loading}
      ...
      <div className="relative w-full z-10">
        ...
        {searchQuery.trim() && (
          <div className="absolute left-0 right-0 top-14 z-50 rounded-xl border border-border bg-slate-950/90 p-4 shadow-2xl backdrop-blur-md max-h-96 overflow-y-auto flex flex-col gap-2">
            <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider px-2">Matched Results ({searchResults.length})</div>
            {searchResults.length === 0 ? (
              <div className="text-center py-6 text-slate-500 italic text-xs">No matching articles found. Try another query.</div>
            ) : (
              searchResults.map((doc) => (
                ...
                >
                  ...
                  {doc.isSop && (
                    <span className="ml-auto text-[9px] font-semibold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded font-mono">SOP</span>
                  )}
                </Link>
              ))
            )}
      ...
      <div className="flex flex-col gap-4">
        ...
        {isCreatingSpace && (
          <form onSubmit={handleCreateSpaceSubmit} className="border border-border/20 bg-slate-950/20 backdrop-blur-md p-6 rounded-xl flex flex-col gap-4 max-w-xl">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">New Space Form</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {spaces.map((space) => {
            const iconEmoji = space.icon === 'book-open' ? '📖' : space.icon === 'user-plus' ? '👥' : space.icon === 'shield-alert' ? '🔒' : '📁';
            return (
              <Link
                key={space._id}
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="text-2xl group-hover:scale-110 transition-transform duration-200">{iconEmoji}</span>
                  <span className="text-sm font-semibold text-slate-200 group-hover:text-emerald-400 transition-colors truncate">{space.name}</span>
                </div>
                <p className="text-xs text-slate-400 mt-2.5 leading-relaxed line-clamp-2">{space.description || 'Central operational wikis and guidelines.'}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-4">
        ...
        <div className="border border-border/20 bg-slate-950/10 backdrop-blur-md p-5 rounded-xl flex flex-col gap-3">
          <h3 className="text-sm font-semibold text-slate-300 flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-emerald-400" /> Compliance Sign-offs</h3>
          <div className="flex-1 flex flex-col justify-center items-center py-6 gap-2">
            <div className="text-4xl font-extrabold text-emerald-400 font-mono">{analytics?.complianceRate || 100}%</div>
            ...
            <div className="w-full bg-slate-900 border border-border/40 h-2.5 rounded-full overflow-hidden mt-2">
              <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${analytics?.complianceRate || 100}%` }} />
            </div>
          </div>
        </div>
        ...
        <div className="border border-border/20 bg-slate-950/10 backdrop-blur-md p-5 rounded-xl flex flex-col gap-3">
          <h3 className="text-sm font-semibold text-slate-300 flex items-center gap-1.5"><AlertTriangle className="h-4 w-4 text-amber-500 animate-pulse" /> Stale Documents Alert</h3>
          <div className="flex-1 flex flex-col gap-2 overflow-y-auto max-h-48 pr-1">
            {(!analytics?.staleDocs || analytics.staleDocs.length === 0) ? (
              <div className="text-center py-10 text-slate-500 italic text-xs">All playbooks up to date. Zero stale pages!</div>
            ) : (
              analytics.staleDocs.map((doc: any) => (
                ...
                >
                  ...
                  <span className="text-[9px] text-amber-500 font-mono font-semibold">Needs Review</span>
                </Link>
              ))
            )}
          </div>
        ...
        <div className="border border-border/20 bg-slate-950/10 backdrop-blur-md p-5 rounded-xl flex flex-col gap-3">
          <h3 className="text-sm font-semibold text-slate-300 flex items-center gap-1.5"><Activity className="h-4 w-4 text-emerald-400" /> Contribution Feed</h3>
          <div className="flex-1 flex flex-col gap-2 overflow-y-auto max-h-48 pr-1">
            {(!analytics?.latestActivities || analytics.latestActivities.length === 0) ? (
              <div className="text-center py-10 text-slate-500 italic text-xs">No collaborative activities logged yet.</div>
            ) : (
              analytics.latestActivities.map((act: any) => {
                const author = act.userId?.name || 'Workspace Member';
                return (
                  <div key={act._id} className="flex flex-col gap-0.5 border-b border-border/10 pb-1.5 text-[11px]">
                    <div className="flex items-center gap-1.5">
                      <div className="h-4 w-4 rounded-full bg-slate-950/60 flex items-center justify-center text-slate-500 border border-border"><User className="h-2.5 w-2.5" /></div>
                      <span className="font-semibold text-slate-300">{author}</span>
                      <span className="text-[9px] text-slate-500 ml-auto">
                        {act.createdAt ? new Date(act.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'now'}
                      </span>
                    </div>
