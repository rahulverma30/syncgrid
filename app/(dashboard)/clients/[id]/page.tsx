...
export default function ClientDetailsPage() {
  ...
  const fetchClients = useCallback(async () => {
    try {
      ...
      if (d.success) setSelectedClient(d.data);
    } catch {}
  }, [id]);
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto space-y-6 pb-20 pt-6">
        <div className="flex items-center justify-between gap-4 select-none">
          <button
            onClick={() => router.push('/clients')}
            className="h-8 hover:bg-accent/40 text-xs gap-1 border border-border/50 bg-background/50 px-3 flex items-center justify-center rounded-md font-medium text-muted-foreground hover:text-foreground transition-colors"
          >
        ...
        <div className="bg-gradient-to-r from-slate-900/40 to-slate-950/40 border border-border/60 p-6 rounded-2xl backdrop-blur-md shadow-xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-400 border border-blue-500/20 text-2xl font-black font-mono shadow-inner select-none">
                <Activity className="h-8 w-8 text-blue-400/80" />
              </div>
        ...
        {/* Sub-tabs selectors */}
        <div className="flex border-b border-border/40 pb-1 flex-wrap gap-2 select-none">
          {['overview', 'contacts', 'contracts', 'communication', 'vault', 'notes', 'timeline'].map(
            (t) => (
              <button
                key={t}
                onClick={() => setActiveTab(t)}
                className={`px-4 py-2 rounded-t-lg text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer border-b-2 ${
                  activeTab === t
        ...
        <div className="p-2 space-y-6">
          ...
          {activeTab === 'contacts' && (
            ...
            >
              ...
              {/* Contacts lists */}
              <div className="space-y-2 border-t border-border/30 pt-4">
                {selectedClient?.contacts?.map((c: any, idx: number) => (
                  <div
                    key={idx}
          ...
          {activeTab === 'contracts' && (
            ...
            >
              ...
              <div className="space-y-2 border-t border-border/30 pt-4 text-left">
                {selectedClient?.contracts?.map((con: any, idx: number) => (
                  <div
                    key={idx}
          ...
          {activeTab === 'vault' && (
            ...
            >
              ...
              <div className="space-y-2 border-t border-border/30 pt-4">
                {selectedClient?.documents?.map((file: any, idx: number) => (
                  <div
                    key={idx}
                ...
                {selectedClient?.timeline
                  ...
                  .map((evt: any, idx: number) => (
                    <div key={idx} className="relative space-y-1 text-xs">
                      <span className="absolute -left-[27px] top-0.5 rounded-full border border-border bg-background p-1 flex items-center justify-center shadow-sm text-primary">
