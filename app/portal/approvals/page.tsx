...
/**
 * Due Date Countdown Alert Component
 */
function DueDateAlert({ dueDate, status }: { dueDate: string | null; status: string }) {
  const [alertText, setAlertText] = useState<string>('');
  const [colorClass, setColorClass] = useState<string>('text-slate-500');

  useEffect(() => {
    if (!dueDate || status !== 'pending') return;
    ...
    if (diff < 0) {
      setAlertText('Overdue!');
      setColorClass('text-rose-500 font-bold border-rose-500/25 bg-rose-500/10 animate-pulse');
    } else if (days < 2) {
      setAlertText(`Urgent: ${days}d ${hours}h left`);
      setColorClass('text-amber-400 font-bold border-amber-500/20 bg-amber-500/5');
    } else {
      setAlertText(`Due in ${days} days`);
      setColorClass('text-slate-400 border-slate-800 bg-slate-900/40');
    }
  }, [dueDate, status]);

  if (!dueDate || status !== 'pending') return null;

  return (
    <span
      className={`inline-flex items-center text-[9px] px-2 py-0.5 rounded border capitalize ${colorClass}`}
    >
...
export default function PortalApprovalsPage() {
  ...
  const healthScore = totalCount > 0 ? Math.round((approvedCount / totalCount) * 100) : 100;
  ...
  // Wire real-time SSE listener
  useEffect(() => {
    const unsubscribe = registerListener('approval_update', () => {
      fetchApprovals();
    });
    return () => unsubscribe();
  }, [registerListener, selectedApproval]);

  const handleAction = async (action: 'approved' | 'revision_requested') => {
    if (!selectedApproval) return;
    if (action === 'approved' && !signatureVerified) {
      toast.error('You must certify the authorized digital signature checkmark to approve.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/portal/approvals', {
        ...
        body: JSON.stringify({
          ...
          action,
          comments,
          signatureVerified: action === 'approved' ? true : false,
        }),
      });
      const body = await res.json();
      if (res.ok && body.success) {
        toast.success(
          `Action successfully recorded: ${action === 'approved' ? 'Approved & Signed Off' : 'Revision Requested'}`
        );
        setComments('');
  ...
  return (
    <div className="space-y-6 text-left">
      {/* Cabinet Stats overview banner */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-slate-900/40 border-slate-850 p-4 rounded-2xl flex items-center space-x-3.5">
          ...
          <div>
            ...
            <span className="text-xl font-bold text-white block mt-0.5">{totalCount} Requests</span>
          </div>
        </Card>
          ...
          <div className="space-y-3">
            {approvals.map((req) => {
              ...
              return (
                <div
                  key={req._id}
                  className={`cursor-pointer transition-all duration-350 rounded-2xl border p-5 space-y-3 relative overflow-hidden text-left ${
                    isSelected
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex items-center space-x-2">
                      <DueDateAlert dueDate={req.dueDate} status={req.status} />
        ...
        {selectedApproval && (
          <div className="lg:col-span-2 space-y-6">
            <Card className="bg-slate-900/40 border-slate-850 rounded-3xl p-6 lg:p-8 space-y-6">
              ...
              {/* Shared Files download simulation */}
                ...
                {selectedApproval.history.length === 0 ? (
                  ...
                  <div className="relative border-l border-slate-850 ml-3 pl-6 space-y-4 pt-2">
                    {selectedApproval.history.map((hist: any, index: number) => {
                      const hasSignature =
                        hist.comments &&
                        hist.comments.includes('[SECURE DIGITAL SIGNATURE CERTIFIED');
                      const cleanComments = hasSignature
                        ? hist.comments.replace(
                      ...
                      return (
                        <div key={index} className="relative">
                          ...
                          <div className="space-y-1">
                            ...
                            {hasSignature && (
                              <div className="inline-flex items-center space-x-1.5 text-[9px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded mt-1.5">
                                <ShieldCheck className="w-3 h-3" />
                                <span>Secured Cryptographic Digital Signature Verified</span>
                              </div>
                            )}
              ...
              {selectedApproval.status === 'pending' && (
                <div className="space-y-4 border-t border-slate-850 pt-6 text-left">
                  ...
                  {/* Cryptographic Digital Signature Verification check */}
                  <label className="flex items-start space-x-3 bg-slate-950/40 border border-slate-850/60 p-4 rounded-2xl cursor-pointer hover:border-slate-800 transition-colors">
                    <input
                      type="checkbox"
                      checked={signatureVerified}
                      onChange={(e) => setSignatureVerified(e.target.checked)}
                      className="mt-0.5 w-4 h-4 text-blue-600 bg-slate-900 border-slate-800 rounded focus:ring-blue-500 focus:ring-offset-slate-900 cursor-pointer"
                      disabled={isSubmitting}
                    />
                    <div className="text-xs text-slate-400 select-none">
                      <span className="font-bold text-white block">
                        Authorized Digital Signature Certification
                      </span>
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        I certify that I am authorized to sign off on this deliverable, and my
                        action constitutes a binding digital signature.
                      </p>
                    </div>
                  </label>
                  ...
                  <div className="flex items-center justify-end space-x-4 pt-2">
                    ...
                    <Button
                      className={`rounded-xl py-5 px-6 border-0 shadow-lg font-semibold text-xs transition-all ${
                        signatureVerified
                      ...
                      disabled={isSubmitting || !signatureVerified}
                    >
                      {isSubmitting ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
