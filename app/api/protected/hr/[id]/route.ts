...
export const GET = withApiPermission(
  ...
  async (request: Request, context: any, session: any) => {
    try {
      ...
      // Dynamic Permission: HR Managers/Admins or the employee themselves can see full compensation details
  ...
  },
  {
    getContext: async (request, context) => {
      ...
      const params = await context.params;
      const { id } = params;
      const employee = await Employee.findById(id).select('userId companyId').lean();
      return {
...
export const PUT = withApiPermission(
  ...
  async (request: Request, context: any, session: any) => {
    try {
      ...
      // Check dynamic dynamic permission for HR access
      const hasHrAccess = hasPermission(session.user.permissions || [], 'hr', 'manage');
