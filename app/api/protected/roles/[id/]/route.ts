  ...
  try {
    ...
    if (!isAdmin) {
      await AuditLog.create({
        ...
        userAgent,
        status: 'failure',
        metadata: { attemptedAction: 'update_role' },
      });
    ...
    if (role.isSystem || role.isSystemRole) {
      return NextResponse.json(
        { success: false, message: 'System role parameters are read-only and cannot be mutated.' },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (!isAdmin) {
      await AuditLog.create({
        ...
        userAgent,
        status: 'failure',
        metadata: { attemptedAction: 'delete_role' },
      });
