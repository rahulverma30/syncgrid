...
import { SprintIngestSchema } from '@/lib/validators/project';
...
export const POST = withApiAuth(async (request: Request, context: any, session: any) => {
  try {
    ...
    const parseResult = SprintIngestSchema.safeParse(body);
    ...
    project.sprints.push(parseResult.data);
    ...
    const activity = new ProjectActivity({
      ...
      description: `Sprint "${parseResult.data.name}" created by ${userName}.`,
      userName,
    });
    await activity.save();
    await project.save();
