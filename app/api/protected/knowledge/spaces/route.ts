...
export const POST = withApiAuth(async (request: Request, context: any, session: any) => {
  try {
    ...
    if (!name) {
      return NextResponse.json({ success: false, message: 'Space name required' }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json({ success: false, message: 'Space with similar name already exists' }, { status: 400 });
    }
