...
/**
 * CenteredModal with glassmorphism, backdrop blurs, and quick Esc closing hooks
 */
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className={cn(
              'relative z-10 w-full max-w-lg rounded-xl border border-border bg-card shadow-2xl p-6 flex flex-col max-h-[90vh] overflow-y-auto select-none',
              className
            )}
          >
            {/* Header */}
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto pr-1 text-sm text-foreground mb-6 leading-relaxed">
              {children}
            </div>

            {/* Footer */}
            {footer && (
              <div className="flex items-center justify-end gap-3.5 border-t border-border/40 pt-4 select-none">
                {footer}
              </div>
            )}
          </motion.div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <motion.div
            ...
            className={cn(
              'relative z-10 w-full max-w-xl rounded-t-2xl border-t border-border bg-card p-6 flex flex-col max-h-[85vh] overflow-y-auto shadow-2xl pb-8',
              className
            )}
          >
            {/* Grab Handle */}
            <div className="mx-auto h-1.5 w-12 rounded-full bg-muted-foreground/35 mb-4 select-none" />

            {/* Header */}
            <div className="flex items-center justify-between pb-4 mb-4 select-none border-b border-border/40">
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto text-sm text-foreground mb-6 leading-relaxed">
              {children}
            </div>

            {/* Footer */}
            {footer && (
              <div className="flex items-center justify-end gap-3 border-t border-border/40 pt-4 select-none">
                {footer}
              </div>
            )}
          </motion.div>
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-border/80 pb-4 mb-6 select-none">
          ...
          <div
            className={cn(
              'flex-1 overflow-y-auto w-full text-sm text-foreground leading-relaxed',
              className
            )}
          >
            {children}
          ...
          {/* Footer */}
          {footer && (
            <div className="flex items-center justify-end gap-3 border-t border-border/80 pt-4 mt-6 select-none">
              {footer}
            </div>
          )}
        </motion.div>
